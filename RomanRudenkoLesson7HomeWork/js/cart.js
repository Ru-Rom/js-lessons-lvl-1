...
const cartRemoves = document.querySelector('.cart-remove-column');
const removeFromCart = document.getElementsByClassName('cart-remove-button');
...
function addRemoveButtonsListeners() {
    Array.from(removeFromCart).forEach(function(element) { // вешаем прослушку на все кнопки
        element.addEventListener('click', function() {
            // debug
            console.log(element);
            removeItemFromCart(element.getAttribute('data-id')); // получаем id нажатой кнопки, являющийся и id товара и вызываем функцию добавления в корзину
        });
    });
}
...
function removeItemFromCart(item) {
        console.log('item: ' + item);   
            ...
            countCartPrice(itemsInCart); // пересчитываем товары в корзине после удаления
            cartList(itemsInCart); // перерисовываем блок корзины
...
function cartList(cartItems) {
    cartPrices.innerHTML = cartNames.innerHTML = cartRemoves.innerHTML = '';

    let itemDivName, itemDivPrice, removeButton, addButton, itemName, itemPrice;
    for (let i = 0; i < cartItems.length; i++) {
        ...
        cartRemoves.appendChild(removeButton);
        removeButton.insertAdjacentHTML('afterbegin', '<i class="far fa-times-circle"></i>');
        removeButton.className = 'cart-remove-button';
        removeButton.setAttribute('data-id', i);

        (i === cartItems.length - 1) ? (addRemoveButtonsListeners()) : null; // когда достигнут конец списка товаров, вызываем функцию добавления прослушки событий клика на кнопку
    }
